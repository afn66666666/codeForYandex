...
GameWinScreen::GameWinScreen(QWidget*parent):
    QGraphicsView(parent)
{
    ...
    this->setScene(scene);
    connect(timer,&QTimer::timeout,this,&GameWinScreen::slotReturnToMenu);
    Retimer->start(0);
    connect(Retimer,&QTimer::timeout,this,&GameWinScreen::slotGameWIN);
}
...
void GameWinScreen::RecordInFile(int Scores)
{
    std::ofstream out("Records.txt",std::ios::app);
    std::ifstream inp("Records.txt");
}

void GameWinScreen::slotGameWIN()
{
    ...
    QGraphicsTextItem * StringStage = new QGraphicsTextItem("you win !");
...
void GameWinScreen::slotReturnToMenu()
{// ВОЗВРАТ В ГЛАВНОЕ МЕНЮ И ЗАПИСЬ ОЧКОВ В ФАЙЛ
